...
if ( ! function_exists( 'cb_top_nav_right' ) ) {
    function cb_top_nav_right( $cb_location = NULL ){
        ...
        $cb_menu_output = $cb_s_tooltip = $cb_log_tooltip = NULL;
        if ( ( $cb_top_nav_login != 'off' ) || ( $cb_top_nav_search != 'off' ) ) {
            ...
            if ( $cb_menu_extras_title == 'cb-menu-extras-title-tooltip' ) {
                ...
                $cb_log_tooltip = 'class="cb-tip-bot"  data-cb-tip="' . $cb_login_title_text . '"';
            ...
            if ( $cb_top_nav_search == 'on' ) {
                $cb_menu_output .=  '<li class="cb-icon-search"><a href="#" ' . $cb_s_tooltip . ' title="' .  __('Search', 'cubell') . '" id="cb-s-trigger"><i class="fa fa-search"></i>' . $cb_s_text . '</a></li>';
            }

            if ( function_exists('login_with_ajax') ) {
                 if ( $cb_top_nav_login == 'on' ) {
                    $cb_menu_output .= $cb_login_class . '<a href="#" ' . $cb_log_tooltip . ' title="' . esc_attr( $cb_login_title_text ) . '" id="cb-lwa-trigger">' . $cb_login_space . ' ' . $cb_login_title . '</a></li>';
                 }
            }
            if ( $cb_location == 'cb-top' ) {
                $cb_menu_output .= '</ul>';
...
if ( ! function_exists( 'cb_ot_header_logo_link' ) ) {
    function cb_ot_header_logo_link() { 
        return '<img src="' .  get_template_directory_uri()  . '/library/admin/images/logo.png"> <span class="cb-version">Theme Options ' . CB_VER . '</span>';
    }
}
...
if ( ! function_exists( 'cb_review_boxes' ) ) {
    function cb_review_boxes( $post ){
      ...
      <meta itemprop="bestRating" content="' . $cb_best_rating . '"><meta itemprop="worstRating" content="0"></div>'; }
                    ...
                    $cb_reader_rating = '<div class="cb-bar cb-font-header cb-review-area clearfix cb-user-rating' . $cb_star_bar . '" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating"><div id="cb-vote" class="' . $cb_star_bar. ' ' . $cb_score_display_type . $cb_class . $cb_tip_class . '" data-cb-tip="' . $cb_voted_text . '" ' . $cb_tip_title . ' data-cb-nonce="' . wp_create_nonce( 'cburNonce' ) . '"><span class="cb-criteria" data-cb-text="' . $cb_rating_text . '">' . __( "Reader Rating", "cubell" ) . ' <span class="cb-votes-count" itemprop="ratingCount">' . $cb_number_votes . ' ' . $cb_vote_votes . '</span></span>';
                } else {
