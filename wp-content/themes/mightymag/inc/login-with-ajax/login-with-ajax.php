...
class LoginWithAjax {
	...
	public static function init(){
		...
		if ( !empty($_REQUEST["lwa"]) ) { //AJAX Request
			...
			add_shortcode('mightymag', 'LoginWithAjax::shortcode');
	...
	public static function login(){
		...
		if( !empty($_REQUEST['log']) && !empty($_REQUEST['pwd']) && trim($_REQUEST['log']) != '' && trim($_REQUEST['pwd'] != '') ){
			...
			if ( strtolower(get_class($loginResult)) == 'wp_user' ) {
				...
				$return['result'] = true;
				$return['message'] = __("Login Successful, redirecting...",'mightymag');
				...
				if( !empty(self::$data['no_login_refresh']) && self::$data['no_login_refresh'] == 1 ){
					...
					$return['message'] = __("Login successful, updating...",'mightymag');
				}
				...
				//Undefined Error
				$return['result'] = false;
				$return['error'] = __('An undefined error has ocurred', 'mightymag');
			}
		}else{
			$return['result'] = false;
			$return['error'] = __('Please supply your username and password.', 'mightymag');
		}
		$return['action'] = 'login';
	...
	public static function register(){
		...
		if ( !is_wp_error($errors) ) {
			//Success
			$return['result'] = true;
			$return['message'] = __('Registration complete. Please check your e-mail.','mightymag');
	...
	public static function remember(){
		...
		if ( $result === true ) {
			...
			$return['result'] = true;
			$return['message'] = __("We have sent you an email", 'mightymag');
	...
	public static function new_user_notification($user_login, $plaintext_pass, $user_email, $blogname){
		...
		$message = str_replace('%BLOGURL%', site_url(), $message);

		$subject = self::$data['notification_subject'];
		$subject = str_replace('%BLOGNAME%', $blogname, $subject);
		$subject = str_replace('%BLOGURL%', site_url(), $subject);
...
//	load_plugin_textdomain('mightymag', false, "login-with-ajax/langs");
//}
