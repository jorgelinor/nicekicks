...
class LoginWithAjaxAdmin{
	...
	function options() {
		...
		if( !empty($_POST['lwasubmitted']) && current_user_can('list_users') && wp_verify_nonce($_POST['_nonce'], 'login-with-ajax-admin'.get_current_user_id()) ){
			...
			?>
			<div class="updated"><p><strong><?php _e('Changes saved', 'login-with-ajax'); ?></strong></p></div>
			<?php
		}else{
...
The team at %BLOGNAME%', 'login-with-ajax');
							...
							<div>
								...
								<p class="submit">
									<input type="submit" class="button-primary" value="<?php _e('Save Changes','login-with-ajax') ?>" />
								</p>
							</div>		
						</form>
					</div>
