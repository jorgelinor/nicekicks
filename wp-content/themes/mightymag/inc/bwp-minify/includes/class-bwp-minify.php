...
class BWP_MINIFY extends BWP_FRAMEWORK {
	...
	function build_option_pages()
	{
		if (!current_user_can(BWP_MINIFY_CAPABILITY))
			wp_die(__('You do not have sufficient permissions to access this page.', 'mightymag-admin'));
...
{	
	if ($page == BWP_MINIFY_OPTION_GENERAL)
	{
		...
		if (!file_exists($options['input_cache_dir']) || !is_writable($options['input_cache_dir']))
			$this->add_notice('<strong>' . __('Warning', 'mightymag-admin') . ':</strong> ' . __("Cache directory does not exist or is not writable. Please try CHMOD your cache directory to 755. If you still see this warning, CHMOD to 777.", 'bwp-minify'));
...
<?php
	...
	function get_buster($type)
	{
		...
		switch ($type)
		{
			...
			case 'tver':
				$theme =  wp_get_theme(STYLESHEETPATH . '/style.css');
	...
	{
		$url = @parse_url($src);
		$blog_url = @parse_url( home_url() );/* djwd get_option(home) -> home_url */
		if (false === $url)
			return false;
		// If scheme is set
		if (isset($url['scheme']))
